...
export default function Level1_Standard({ 
  ...
  useEffect(() => {
    ...
    csvData.forEach(cat => {
        cat.words.forEach(w => {
            newTiles.push({ id: Math.random().toString(36).substr(2, 9), word: w, categoryId: cat.id, categoryName: cat.name.includes(':') ? cat.name.split(':')[1].trim() : cat.name, status: 'neutral', isSolved: false });
        });
    });
    ...
    if (selectedId === null) {
      ...
      setTimeout(() => {
          setTiles(p => {
              ...
              n[idx1] = { ...t1, word: t2.word, categoryId: t2.categoryId, categoryName: t2.categoryName };
              n[idx2] = { ...t2, word: t1.word, categoryId: t1.categoryId, categoryName: t1.categoryName };
              return n;
          });
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      for (let r = 0; r < tiles.length / 4; r++) {
        ...
        // Check if row is already fully matched (ready for checkMatches) but not marked solved
        if (row.every(t => t.categoryId === targetCatId)) {
          checkMatches(tiles);
          return;
        }

        // Find a tile in this row that DOES NOT belong
        const wrongTileIdx = row.findIndex(t => t.categoryId !== targetCatId);
        if (wrongTileIdx !== -1) {
          const wrongTile = row[wrongTileIdx];
          const globalIdx = r * 4 + wrongTileIdx;
          
          // Find where one of the correct tiles is hiding
