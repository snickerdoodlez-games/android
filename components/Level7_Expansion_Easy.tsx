...
const Level7_Expansion_Easy: React.FC<any> = ({ 
  ...
  const getSolvedRowCount = useCallback(() => {
    let count = 0;
    for (let rIdx of activeRowIndices) { const row = activeColIndices.map(cIdx => gridData[rIdx][cIdx]); if (row.every(t => t?.status === 'solved')) count++; }
    return count;
  ...
  const checkMatches = useCallback((currentGrid: (TileData | null)[][], wasSwap: boolean = false) => {
    ...
    if (currentNewlySolved === activeRows.length) {
  ...
  const performHint = useCallback(() => {
    ...
    for (const rIdx of shuffledRows) {
      ...
      const row = activeCols.map(cIdx => currentGrid[rIdx][cIdx]); if (row.some(t => !t) || row.every(t => t!.status === 'solved')) continue;
      const targetCatId = csvData[rIdx]?.id; if (!targetCatId) continue;
      ...
      for (const sr of otherRows) { for (const sc of activeCols) { const t = currentGrid[sr][sc]; if (t && t.categoryId === targetCatId && t.status !== 'solved' && t.status !== 'locked') matchIndices.push({ r: sr, c: sc }); } }
               ...
               <div key={rIdx} className="flex-1 relative min-h-0 overflow-visible">
                  ...
                  {solved && (
                    <div className="absolute top-0 left-6 z-[100] transform -translate-y-full">
                      <div className="px-5 py-2 text-[clamp(0.75rem,3vw,0.9375rem)] font-black uppercase bg-black border-2 border-white text-white rounded-t-lg shadow-[0_-4px_12px_rgba(0,0,0,0.8)] whitespace-nowrap"><CategoryTabLabel name={firstTile?.categoryName || ''} catDict={csvData?.find((r: any) => r.id === firstTile?.categoryId)?.catDict || ''} /></div>
                    </div>
                  )}
