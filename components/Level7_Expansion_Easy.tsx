...
const Level7_Expansion_Easy: React.FC<LevelProps> = ({
  ...
  useEffect(() => {
    ...
    if (!validateCategories(EXPANSION_TEMP_DATA)) {
      console.error('Level7_Expansion_Easy: Invalid category data detected. Level will not initialize.');
      return;
    }
  ...
  const expandGrid = useCallback(() => {
    ...
    setGridData(prev => {
      ...
      nonSolvedIndices.forEach(([r, c], i) => {
        if (i < scrambled.length) next[r][c] = scrambled[i];
        else next[r][c] = null;
      });
      return next;
    });
  ...
  const handleLongPress = useCallback((word: string, definition?: string) => {
    ...
    audio.playSelect();
    const neonColors = ['bg-neon-red', 'bg-neon-orange', 'bg-neon-yellow', 'bg-neon-lime', 'bg-neon-green', 'bg-neon-mint', 'bg-neon-cyan', 'bg-neon-sky-blue', 'bg-neon-blue', 'bg-neon-violet', 'bg-neon-purple', 'bg-neon-magenta', 'bg-neon-pink', 'bg-neon-rose'];
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      for (let rIdx of activeRowIndices) {
        ...
        // Find the most common category in this row (that's the target)
        ...
        {activeRowIndices.map(rIdx => {
          ...
          return (
            <div key={rIdx} className="flex-1 relative min-h-0 overflow-visible">
              {solved && <SolvedRowBackground seed={firstTile?.categoryId || rIdx} colorClass={rowColor} animationKey="expansion-easy" />}
              {solved && (
