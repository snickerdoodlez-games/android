      ...
      {showDef && catDict && createPortal(
        ...
        <m.div
          ...
          <m.div
            ...
            <h2 className="text-[clamp(1.25rem,5vw,2rem)] font-black font-raleway uppercase definition-text text-center tracking-wider leading-tight mb-1 per-letter-neon animate-text-reveal" style={{ animationDelay: '0.1s' }}>{name}</h2>
            <div className="w-12 h-0.5 bg-neon-aqua mx-auto my-4 shadow-[0_0_8px_#00FFFF]" />
            <p className="text-[clamp(0.8125rem,3.5vw,1.125rem)] leading-relaxed definition-text font-raleway text-center whitespace-normal break-words max-w-[65ch] mx-auto animate-text-reveal" style={{ animationDelay: '0.5s' }}>{catDict}</p>
            <div className="mt-6 mb-4 w-full h-px bg-zinc-800" />
            <button className="block mx-auto px-8 py-2 min-w-[48px] min-h-[48px] bg-white text-black font-black font-raleway text-[clamp(0.75rem,3vw,1rem)] uppercase rounded-medium hover:scale-105 active:scale-95 transition-all" onClick={() => setShowDef(false)} aria-label="Close definition overlay">TAP TO CLOSE</button>
          </m.div>
        </m.div>
