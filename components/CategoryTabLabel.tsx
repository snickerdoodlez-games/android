  ...
  return (
    <>
      ...
      {showDef && catDict && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80" onClick={() => setShowDef(false)}>
          <div className="bg-black border-2 border-white rounded-large px-8 py-8 mx-6 shadow-[0_0_40px_rgba(0,255,255,0.5)] max-w-[90vw] w-full" style={{ maxWidth: '400px' }} onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <h2 className="text-[clamp(1.25rem,5vw,2rem)] font-black font-raleway uppercase text-white text-center tracking-wider leading-tight mb-1">{name}</h2>
            <div className="w-12 h-0.5 bg-neon-aqua mx-auto my-4 shadow-[0_0_8px_#00FFFF]" />
            <p className="text-[clamp(0.8125rem,3.5vw,1.125rem)] leading-relaxed text-white font-raleway text-center whitespace-normal break-words max-w-[65ch] mx-auto">{catDict}</p>
