...
const NextLevelButton: React.FC<Props> = ({ onClick, label = "NEXT PUZZLE", isMain = false, themeColors, levelIndex }) => {
  ...
  // Keep theme colors for the glow effect if desired, but button itself is black
  ...
  return (
    <div className="relative group cursor-pointer p-4 flex flex-col items-center justify-center" onClick={onClick}>
      {/* Outer Glow Ring - Adjusted to rounded-small to match tile shape */}
      ...
      {/* Dynamic Color Glow - Adjusted to rounded-small */}
      ...
      {/* Main Button Body - Tile Styling */}
      <button 
        className="relative px-12 py-4 md:px-16 md:py-6 bg-black border-4 border-white rounded-small flex items-center justify-center overflow-hidden transform transition-all duration-200 hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,1)] z-20 min-w-[280px] min-h-[70px]"
      >
        ...
        {/* Text Container with Arcade Outline */}
        <div 
          className="flex flex-col items-center justify-center z-30 transition-opacity duration-300 ease-in-out"
          style={{ opacity: opacity }}
        >
            <span 
              className="text-2xl md:text-3xl font-bold font-oswald uppercase tracking-widest text-white whitespace-nowrap"
      ...
      {!isMain && (
         <p className="mt-3 text-zinc-500 text-[10px] font-bold uppercase tracking-[0.2em] animate-pulse">click to review puzzle</p>
      )}
    </div>
  );
};
