...
export default function Level_Themed({ 
  ...
  const handleTileClick = useCallback((id: string) => {
    ...
    if (firstId === null) {
      audio.playSelect(); selectedIdRef.current = id; setSelectedId(id);
    ...
    isSwappingRef.current = true; setIsSwapping(true);
    selectedIdRef.current = null; setSelectedId(null);
    audio.playSwap();
    ...
    setTimeout(() => {
      setTiles(prev => {
        const i1 = prev.findIndex(t => t.id === firstId);
        const i2 = prev.findIndex(t => t.id === secondId);
        if (i1 === -1 || i2 === -1) return prev;
        const next = [...prev];
        const t1 = next[i1];
        const t2 = next[i2];
        next[i1] = { ...t1, word: t2.word, categoryId: t2.categoryId, categoryName: t2.categoryName, definition: t2.definition };
        next[i2] = { ...t2, word: t1.word, categoryId: t1.categoryId, categoryName: t1.categoryName, definition: t1.definition };
        return next;
      });

      setTimeout(() => {
        setTiles(prev => {
          const f = prev.map(t => (t.id === firstId || t.id === secondId) ? { ...t, status: 'neutral' as const } : t);
          setTimeout(() => checkMatches(f), 0);
          return f;
        });
        isSwappingRef.current = false; setIsSwapping(false);
      }, 450);
    }, 50);
  }, [checkMatches]);
  ...
  }, [isAutoPlaying, isComplete, isReviewing, handleTileClick, checkMatches]);
  ...
  // Squish animation when level completes
  ...
  const performHint = useCallback(() => {
    const currentTiles = tilesRef.current;
    if (currentTiles.length === 0 || isCompleteRef.current || isSwappingRef.current) return;
    const rowCount = Math.floor(currentTiles.length / 4);
    const colCount = 4;
  ...
  const rowElements = Array.from({ length: tiles.length / 4 }).map((_, r) => {
    const row = tiles.slice(r * 4, r * 4 + 4);
    ...
    return (
      <div key={r} className={`flex-1 relative min-h-0 overflow-visible ${isSquishing ? 'level-complete-squish' : ''}`}>
        ...
        <div className={`grid grid-cols-4 gap-0.5 w-full h-full relative z-10 transition-all duration-300 ${solved ? 'p-[10px]' : 'p-0.5'}`}>
          {row.map((tile, colIdx) => <Tile key={tile.id} data={tile} onClick={handleTileClick} gridEntryDelay={0.05 + r * 0.1} ref={(el: any) => { if(el) tileRefs.current.set(tile.id, el); }} />)}
        </div>
      </div>
    );
  });
