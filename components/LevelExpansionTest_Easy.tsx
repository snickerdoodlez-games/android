...
const LevelExpansionTest_Easy: React.FC<any> = ({ 
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setGridData(prev => {
            ...
            // Fill any remaining null positions (safety net)
  ...
  const handleTileClick = useCallback((r: number, c: number) => {
    ...
    if (!tile || tile.status === 'solved' || tile.status === 'locked' || tile.status === 'falling-out' || tile.status === 'flipping-out') return;
...
export default LevelExpansionTest_Easy;