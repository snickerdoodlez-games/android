...
function getCurrentVersion() {
    ...
    if (capConfig && capConfig.appVersion)
        return String(capConfig.appVersion);
...
function findAdMobIds() {
    ...
    for (const dir of searchDirs) {
        ...
        for (const file of files) {
            if (!/\.(ts|tsx|js|jsx|json|xml|gradle|kt|java)$/i.test(file))
                continue;
            const filePath = join(dir, file);
            try {
                if (statSync(filePath).size > 500_000)
...
function validateDistDirectory() {
    ...
    if (distContents.includes("index.html"))
        found.push("index.html");
    else
        missing.push("index.html");
    if (distContents.includes("assets"))
        found.push("assets/");
    else
        warnings.push("assets/ directory not found - may be embedded in JS bundle");
    // Check JS bundle
    ...
    if (cssFiles.length > 0)
        found.push(`${cssFiles.length} CSS file(s)`);
    // Check for Vite manifest
    if (distContents.includes(".vite"))
        found.push(".vite/ manifest");
    // Summary
    return {
...
async function handleValidateCapacitorConfig() {
    ...
    if (existsSync(indexPath)) {
        ...
        if (html.includes("tailwindcss"))
            results.push("   🎨 Tailwind CSS CDN linked");
        if (html.includes("Oswald"))
            results.push("   🔤 Oswald font loaded");
    }
    else {
...
async function handleCheckAdMobIds(args) {
    ...
    if (bannerMatch)
        results.push("   📢 Banner ad integration found");
    if (interstitialMatch)
        results.push("   📺 Interstitial ad integration found");
...
async function handleVerifyDist() {
    ...
    if (distCheck.valid) {
        ...
        for (const m of distCheck.missing)
            results.push(`   - ${m}`);
    }
    if (distCheck.warnings.length > 0) {
        for (const w of distCheck.warnings)
...
async function handleFullBuildAudit(args) {
    ...
    const issues = [];
    if (!existsSync(DIST_DIR))
        issues.push("❌ No build output (dist/)");
    if (csvPoolFiles.length === 0)
        issues.push("❌ No pool data files found");
    if (csvThemeFiles.length === 0)
        issues.push("❌ No theme data files found");
    if (issues.length === 0) {
...
async function handleDiagnoseThemeData(args) {
    const levelIndex = args.levelIndex || 6; // Default: first THEMED level is at index 6
    ...
    for (const tf of themeFiles) {
        ...
        for (const line of lines) {
            ...
            if (!trimmed || trimmed.startsWith("//") || trimmed.startsWith("/*") || trimmed.startsWith("export") || trimmed.startsWith("ID,"))
                continue;
    ...
    for (const tf of themeFiles) {
        ...
        for (const line of lines) {
            ...
            if (parts.length >= 15) {
                ...
                if (!themeDiffCounts[theme])
                    themeDiffCounts[theme] = { 1: 0, 3: 0, 5: 0 };
    ...
    const themeCase = levelContent.match(/case GameMode\.LEVEL_THEMED/);
    if (themeCase) {
