        ...
        try {
            ...
            if (isThemeFormat) {
                ...
                for (let j = 0; j < wordDefs.length; j += 2) {
                    ...
                    if (wordDefs[j + 1])
                        definitions.push(wordDefs[j + 1].replace(/^"|"$/g, ""));
                }
...
function loadAllCSVData() {
    ...
    try {
        ...
        const csvFiles = files.filter((f) => (f.startsWith("csvPoolData") || f.startsWith("csvThemeData")) &&
            f.endsWith(".ts"));
...
// Tool Definitions
// ============================================================
const SEARCH_GAME_CONTENT_TOOL = {
...
const ANALYZE_LEVEL_PROGRESSION_TOOL = {
...
const AUDIT_CSV_CONSISTENCY_TOOL = {
...
const FIND_UNUSED_CATEGORIES_TOOL = {
...
// ============================================================
async function handleSearchGameContent(args) {
    ...
    for (const { rows, fileName } of allData) {
        for (const row of rows) {
            if (broadCategoryFilter && row.broadCategory.toLowerCase() !== broadCategoryFilter.toLowerCase())
                continue;
    ...
    if (broadCategoryFilter)
        output += ` in ${broadCategoryFilter}`;
    output += `.\n\n`;
...
async function handleValidateManifest() {
    ...
    if (existsSync(metadataPath)) {
        try {
            ...
            if (!metadata.name)
                issues.push("metadata.json is missing 'name' field.");
            if (!metadata.description)
                issues.push("metadata.json is missing 'description' field.");
        }
        catch (e) {
    ...
    if (existsSync(capConfigPath)) {
        try {
            ...
            if (!capConfig.appId)
                issues.push("capacitor.config.json is missing 'appId'.");
            if (!capConfig.appName)
                issues.push("capacitor.config.json is missing 'appName'.");
            if (!capConfig.webDir)
                warnings.push("capacitor.config.json is missing 'webDir'.");
        }
        catch (e) {
...
async function handleAnalyzeLevelProgression(numLevels) {
...
async function handleAuditCSVConsistency() {
    const allData = loadAllCSVData();
    const issues = [];
    const warnings = [];
...
async function handleFindUnusedCategories(args) {
    ...
    for (const { rows } of allData) {
        for (const row of rows) {
            if (minDifficulty > 0 && row.difficulty < minDifficulty)
                continue;
